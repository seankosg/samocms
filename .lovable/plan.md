...
- 폭은 A4 비율 기준 1240px, 높이는 내용에 따라 늘어나는 세로 1장.
- 저장 위치: 지금 PDF와 같은 보관함에 `safety/2026-09-12-ko.jpg`, `-en.jpg`.
- 한글·영문 분석이 모두 끝나는 순간(자동 생성·「다시 분석」 모두) PDF와 함께 만들어집니다.

### 2) 봇 연계
- 봇이 읽는 공개 목록(`v_safety_telegram`)에 `jpg_ko_url`, `jpg_en_url`을 추가합니다. PDF와 동일하게 `?v=준비시각`이 붙어 옛 파일이 캐시되지 않습니다.
- 인증 없이 받아갈 수 있는 공개 주소 `/api/public/safety-image?day=…&lang=ko|en` 를 새로 만듭니다.
- 봇은 사진 주소로 `sendPhoto`, 설명글에 기존 본문 + PDF 링크를 넣으면 됩니다. (봇 쪽 코드는 앱 범위 밖)
