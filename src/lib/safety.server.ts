...
export const safetyCol = (lang: SafetyLang) =>
  lang === "en" ? { risks: "risks_en", at: "generated_at_en" } : { risks: "risks", at: "generated_at" };

/** 지정 언어로 AI 분석 1회 실행 후 safety_reports 에 저장 */
export async function generateSafety(
  day: string,
  facts: string,
  lang: SafetyLang,
  createdBy: string | null,
): Promise<{ risks: SafetyRisk[]; generatedAt: string }> {
  const key = process.env["LOVABLE_API_KEY"];
  if (!key) throw new Error("AI 키가 설정되어 있지 않습니다.");
  const tc = safetyCol(lang);
  ...
  const { error } = await supabaseAdmin
    .from("safety_reports")
    .upsert({ day, [tc.risks]: risks, [tc.at]: generatedAt, created_by: createdBy } as never, { onConflict: "day" });
